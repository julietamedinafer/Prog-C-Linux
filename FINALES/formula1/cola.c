...
int enviar_mensaje(int id_cola, long receptor, int emisor, int eventoMandar, char* contenidoMsgMandar){
	...
	return msgsnd(id_cola, &msg, sizeof(msg.remitente)+sizeof(msg.evento)+sizeof(msg.contenidoMsg), IPC_NOWAIT);
}

int recibir_mensaje(int id_cola, long destino, mensaje* msgRecibir){
	mensaje msg;
	int res;
	res= msgrcv(id_cola, &msg, sizeof(msg.remitente)+sizeof(msg.evento)+sizeof(msg.contenidoMsg), destino, 0);
...
int borrar_mensaje(int id_cola){
	...
	int res;
	do{
		res= msgrcv(id_cola, &msg, sizeof(msg.remitente) + sizeof(msg.evento) + sizeof(msg.contenidoMsg), 0, IPC_NOWAIT);
	}while(res>0);
